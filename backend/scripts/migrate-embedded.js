...
const SEED_DATA = [
  ...
  // Default admin user (password: iware123)
...
async function runMigration() {
  try {
    ...
    for (let i = 0; i < TABLES.length; i++) {
      try {
        ...
        if (error.message.includes('already exists')) {
          successCount++;
          console.log(`  ✓ Table ${i + 1}/${TABLES.length} already exists`);
        } else {
          console.error(`  ✗ Table ${i + 1}/${TABLES.length} failed:`, error.message);
        }
      }
    }

    console.log(`✅ Database tables ready (${successCount}/${TABLES.length})`);
    ...
    for (let i = 0; i < SEED_DATA.length; i++) {
      try {
        await db.query(SEED_DATA[i]);
        seedCount++;
        console.log(`  ✓ Seed ${i + 1}/${SEED_DATA.length} inserted`);
      } catch (error) {
        if (error.message.includes('Duplicate entry')) {
          seedCount++;
          console.log(`  ✓ Seed ${i + 1}/${SEED_DATA.length} already exists`);
        } else {
          console.error(`  ✗ Seed ${i + 1}/${SEED_DATA.length} failed:`, error.message);
        }
      }
    }

    console.log(`✅ Seed data ready (${seedCount}/${SEED_DATA.length})`);
